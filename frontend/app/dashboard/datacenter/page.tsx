  ...
  return (
    <div className="screen content space-y-10">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        { label: "Réponses reçues", value: stats.totalResponded ?? 0 },
        { label: "Taux d’ouverture", value: `${stats.openRate}%` },
        { label: "Taux de réponse", value: `${stats.responseRate ?? 0}%` }].map((item, idx) => (
      ...
      {stats.variants && stats.variants.length > 0 && (
        <Card className="bg-green-100 dark:bg-green-900/20">
          <CardContent className="p-6">
            <h3 className="text-lg font-semibold text-green-800 dark:text-green-300">🅰️🅱️ Tests A/B</h3>
            <ul className="mt-2 space-y-2">
              {stats.variants.map(v => (
                <li key={v.variant} className="flex justify-between">
                  <span className="font-medium">{v.variant}</span>
                  <span>{v.totalOpened}/{v.totalSent} ouverts ({v.openRate.toFixed(1)}%)</span>
                </li>
              ))}
            </ul>
          </CardContent>
